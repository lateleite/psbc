 ...
 *
 * Note that the GFX12 ISA doc describes other layouts for A/B, but they are
 * identical to the C layout with the exception of the order of the rows
 * (columns for A). And as long as these are swapped in the same way for both A
 * and B, the muladd result will be the same. So we use the C layout for all
 * uses.
 */
...
static unsigned gnm_nir_cmat_bits(struct glsl_cmat_description desc) {
  return glsl_base_type_bit_size(desc.element_type);
}

static unsigned gnm_nir_cmat_length(struct glsl_cmat_description desc,
  ...
  if (params->gfx_level >= GFX12) {
    ...
    return desc.cols * desc.rows / params->wave_size *
           (gnm_nir_cmat_bits(desc) == 16 ? 2 : 1);
  }
}

static unsigned gnm_nir_cmat_length_mul(struct glsl_cmat_description desc,
                                        const lower_cmat_params *params) {
  if (params->gfx_level >= GFX12 || desc.use != GLSL_CMAT_USE_ACCUMULATOR) {
    return 1;
  } else {
    /* For  GFX11 C matrices we have 1 VGPR per element even if the element type
     * is 16bits. So with 8 fp16 elements we implement that with a f16vec16. We
     * then use the coefficient generated by this function to figure out how
     * many elements we really have.
     */
    return gnm_nir_cmat_bits(desc) == 16 ? 2 : 1;
  }
}

static nir_def *gnm_nir_load_cmat(nir_builder *b,
                                  const lower_cmat_params *params,
                                  nir_def *src) {
  nir_deref_instr *deref = nir_def_as_deref(src);
  struct glsl_cmat_description desc = *glsl_get_cmat_description(deref->type);
  return nir_build_load_deref(b, gnm_nir_cmat_length(desc, params),
                              gnm_nir_cmat_bits(desc), src, 0);
}

static const struct glsl_type *
gnm_nir_translate_matrix_type(const struct glsl_type *orig_type,
                              struct hash_table *type_map,
                              const lower_cmat_params *params) {
  ...
  if (entry) {
    ...
    unsigned length = gnm_nir_cmat_length(desc, params);
    ...
    const struct glsl_type *new_elem_type =
        gnm_nir_translate_matrix_type(elem_type, type_map, params);
    ...
    for (unsigned i = 0; i < num_fields; ++i) {
      ...
      const struct glsl_type *new_field_type =
          gnm_nir_translate_matrix_type(field_type, type_map, params);
    ...
    for (unsigned i = 0; i < num_fields; ++i) {
      ...
      fields[i].type =
          gnm_nir_translate_matrix_type(fields[i].type, type_map, params);
    }
...
static nir_def *gnm_get_base_row(nir_builder *b,
  ...
  if (params->gfx_level >= GFX12) {
    ...
    base_row = desc.use == GLSL_CMAT_USE_ACCUMULATOR
...
static nir_def *shuffle_xor_imm(nir_builder *b, nir_def *data, unsigned imm) {
  assert(imm < 64);
  if (imm == 32) {
    /* v_permlane64_b32 */
    ...
    return nir_masked_swizzle_amd(b, data, .swizzle_mask = 0x1f | (imm << 10),
                                  .fetch_inactive = 1);
  } else {
    /* There isn't a single instruction that can do this, but for cooperative
     * matrix opcodes all invocations must be active. So we can split the
     * operation into the two previous cases instead of having to use full width
     * shuffle.
     */
...
static bool lower_cmat_length(nir_builder *b, nir_intrinsic_instr *intr,
                              const lower_cmat_params *params) {
  struct glsl_cmat_description desc = nir_intrinsic_cmat_desc(intr);
  unsigned len =
      gnm_nir_cmat_length(desc, params) / gnm_nir_cmat_length_mul(desc, params);
  nir_def_replace(&intr->def, nir_imm_int(b, len));
  return true;
}

static bool lower_cmat_extract(nir_builder *b, nir_intrinsic_instr *intr,
                               const lower_cmat_params *params) {
  nir_deref_instr *src_deref = nir_src_as_deref(intr->src[0]);
  struct glsl_cmat_description desc =
      *glsl_get_cmat_description(src_deref->type);
  nir_def *src0 = gnm_nir_load_cmat(b, params, intr->src[0].ssa);

  nir_def *index = intr->src[1].ssa;
  index = nir_imul_imm(b, index, gnm_nir_cmat_length_mul(desc, params));
...
static bool lower_cmat_insert(nir_builder *b, nir_intrinsic_instr *intr,
                              const lower_cmat_params *params) {
  nir_def *src1 = gnm_nir_load_cmat(b, params, intr->src[2].ssa);
...
static bool lower_cmat_construct(nir_builder *b, nir_intrinsic_instr *intr,
                                 const lower_cmat_params *params) {
  nir_deref_instr *dst_deref = nir_src_as_deref(intr->src[0]);
  struct glsl_cmat_description desc =
      *glsl_get_cmat_description(dst_deref->type);
  ...
  nir_def *r = nir_replicate(b, elem, gnm_nir_cmat_length(desc, params));
...
static bool lower_cmat_load_store(nir_builder *b, nir_intrinsic_instr *intr,
  ...
  const uint32_t ptr_stride = glsl_get_bit_size(deref->type) / 8 *
  ...
  bool load_acc_as_b =
      is_load && params->gfx_level < GFX12 &&
      desc.use == GLSL_CMAT_USE_ACCUMULATOR && gnm_nir_cmat_bits(desc) == 8 &&
      params->wave_size == 32 && layout == GLSL_MATRIX_LAYOUT_COLUMN_MAJOR;
  if (load_acc_as_b)
  ...
  unsigned length = gnm_nir_cmat_length(desc, params);
  unsigned mul = gnm_nir_cmat_length_mul(desc, params);
  unsigned lanes_per_iter =
      desc.use == GLSL_CMAT_USE_ACCUMULATOR ? params->wave_size : 16;
  nir_def *vars[16];
  if (is_load) {
    if (mul > 1) {
      for (unsigned i = 0; i < length; ++i)
        if (i % mul != 0)
          vars[i] = nir_undef(b, 1, gnm_nir_cmat_bits(desc));
    }
  } else {
    if (params->gfx_level < GFX12 && desc.use != GLSL_CMAT_USE_ACCUMULATOR)
      nir_push_if(b, nir_ilt_imm(b, local_idx, 16));

    nir_def *src = gnm_nir_load_cmat(b, params, &cmat_deref->def);
    for (unsigned i = 0; i < length; ++i)
      vars[i] = nir_channel(b, src, i);
  }

  unsigned idx_bits = deref->def.bit_size;
  nir_def *base_row = gnm_get_base_row(b, desc, params, local_idx);
  ...
  if (layout == GLSL_MATRIX_LAYOUT_COLUMN_MAJOR)
    align_mul = MIN2(16, gnm_nir_cmat_bits(desc) * desc.rows / 8);
  ...
  for (unsigned i = 0; i < length / mul; ++i) {
    ...
    iter_deref = nir_build_deref_cast(b, &iter_deref->def, deref->modes,
                                      ...
                                      gnm_nir_cmat_bits(desc) / 8);
    iter_deref = nir_build_deref_ptr_as_array(b, iter_deref, row_offset);

    if (align_mul) {
      unsigned align_offset =
          row_iter * gnm_nir_cmat_bits(desc) / 8 % align_mul;
      iter_deref = nir_build_deref_cast_with_alignment(
          b, &iter_deref->def, deref->modes, iter_deref->type,
          gnm_nir_cmat_bits(desc) / 8, align_mul, align_offset);
    }
  ...
  if (is_load) {
    ...
    if (load_acc_as_b) {
      /* Shift data in the high bits for the first 16 lanes (it's already there
       * for the other 16.) */
      ...
      for (unsigned i = 0; i < mat->num_components / 4; i++) {
        nir_def *packed =
            nir_pack_32_4x8(b, nir_channels(b, mat, 0xfu << (4 * i)));
    ...
    nir_store_deref(b, cmat_deref, mat,
                    nir_component_mask(mat->num_components));
  } else if (params->gfx_level < GFX12 &&
...
static bool lower_cmat_muladd(nir_builder *b, nir_intrinsic_instr *intr,
                              const lower_cmat_params *params) {
  nir_def *A = gnm_nir_load_cmat(b, params, intr->src[1].ssa);
  nir_def *B = gnm_nir_load_cmat(b, params, intr->src[2].ssa);
  nir_def *C = gnm_nir_load_cmat(b, params, intr->src[3].ssa);
  ...
  struct glsl_cmat_description a_desc =
      *glsl_get_cmat_description(a_deref->type);
  struct glsl_cmat_description b_desc =
      *glsl_get_cmat_description(b_deref->type);

  const nir_cmat_signed cmat_signed_mask = nir_intrinsic_cmat_signed_mask(intr);

  enum glsl_base_type a_element_type = glsl_apply_signedness_to_base_type(
  ...
  enum glsl_base_type b_element_type = glsl_apply_signedness_to_base_type(
      b_desc.element_type, cmat_signed_mask & NIR_CMAT_B_SIGNED);

  nir_def *ret = nir_cmat_muladd_amd(
      b, A, B, C, .saturate = nir_intrinsic_saturate(intr),
...
static nir_def *convert_base_type(nir_builder *b, nir_def *src,
                                  enum glsl_base_type src_type,
                                  enum glsl_base_type dst_type, bool sat) {
  ...
  nir_op op = nir_type_conversion_op(
      nir_get_nir_type_for_glsl_base_type(src_type),
      nir_get_nir_type_for_glsl_base_type(dst_type), nir_rounding_mode_undef);

  return nir_build_alu1(b, op, src);
}

static nir_def *convert_use(nir_builder *b, nir_def *src,
                            enum glsl_cmat_use src_use,
  ...
  if (src_use == GLSL_CMAT_USE_A && dst_use == GLSL_CMAT_USE_ACCUMULATOR) {
    ...
    return convert_use(b, src, GLSL_CMAT_USE_B, GLSL_CMAT_USE_ACCUMULATOR,
                       params);
  } else if (src_use == GLSL_CMAT_USE_ACCUMULATOR &&
             dst_use == GLSL_CMAT_USE_A) {
    src =
        convert_use(b, src, GLSL_CMAT_USE_ACCUMULATOR, GLSL_CMAT_USE_B, params);
  ...
  if (src_use == GLSL_CMAT_USE_ACCUMULATOR && dst_use == GLSL_CMAT_USE_B) {
    ...
    if (src->bit_size == 32) {
      for (int cross32 = params->wave_size == 64; cross32 >= 0; cross32--) {
        ...
        nir_def *low_lanes =
            nir_inverse_ballot_imm(b, low_mask, params->wave_size);
      ...
      /* Same as above, but operate on 32 bits at once, using byte_perm as
       * vectorized bcsel. */
      assert(src->bit_size < 32);
      nir_def *packed = nir_extract_bits(b, components, num_comps, 0,
                                         num_comps / (32 / src->bit_size), 32);
      num_comps /= 32 / src->bit_size;
      for (unsigned i = 0; i < num_comps; i++)
        components[i] = nir_channel(b, packed, i);

      nir_def *low_sel =
          nir_imm_int(b, src->bit_size == 8 ? 0x05010400 : 0x05040100);
      nir_def *high_sel =
          nir_imm_int(b, src->bit_size == 8 ? 0x01050004 : 0x01000504);
      ...
      nir_def *unpacked =
          nir_extract_bits(b, components, num_comps, 0,
                           num_comps * (32 / src->bit_size), src->bit_size);
      num_comps *= 32 / src->bit_size;
      for (unsigned i = 0; i < num_comps; i++)
    ...
    assert(num_comps == 16);
             ...
             dst_use == GLSL_CMAT_USE_ACCUMULATOR) {
    assert(params->gfx_level < GFX12);
    assert(num_comps == 16);
    if (src->bit_size == 32) {
      for (unsigned keep32 = 0; keep32 < ((params->wave_size == 64) ? 2 : 1);
           keep32++) {
        nir_def *keep = nir_inverse_ballot_imm(
            b, keep32 ? UINT32_MAX : 0xffff0000ffffull, params->wave_size);
        num_comps /= 2;
        for (unsigned i = 0; i < num_comps; i++) {
          components[i] =
              nir_bcsel(b, keep, components[i * 2], components[i * 2 + 1]);
        }
      }
    } else {
      /* Same as above, but operate on 32 bits at once, using byte_perm as
      ...
      nir_def *low_sel =
          nir_imm_int(b, src->bit_size == 8 ? 0x06040200 : 0x05040100);
      nir_def *high_sel =
          nir_imm_int(b, src->bit_size == 8 ? 0x07050301 : 0x07060302);
      ...
      for (unsigned keep32 = 0; keep32 < ((params->wave_size == 64) ? 2 : 1);
        ...
        num_comps /= 2;
        for (unsigned i = 0; i < num_comps; i++) {
          components[i] = nir_byte_perm_amd(b, components[i * 2 + 1],
                                            components[i * 2], perm);
        }
      }
    ...
    for (unsigned x_mask = 1; x_mask < num_comps; x_mask *= 2) {
      /* Use separate masks to always keep the masked_swizzle on the first
       * source of v_cndmask. */
      uint64_t mask = 0;
      ...
      nir_def *odd =
          nir_inverse_ballot_imm(b, mask << x_mask, params->wave_size);
    ...
    if (params->gfx_level >= GFX12) {
      /* One component contains 2/4 rows in wave32/64, so we must transpose
       * inside it. */
      for (int cross32 = params->wave_size == 64; cross32 >= 0; cross32--) {
...
static bool lower_cmat_convert_transpose(nir_builder *b,
                                         nir_intrinsic_instr *intr,
                                         const lower_cmat_params *params) {
  b->fp_math_ctrl = nir_intrinsic_fp_math_ctrl(intr);
  nir_deref_instr *dst_deref = nir_src_as_deref(intr->src[0]);
  nir_deref_instr *src_deref = nir_src_as_deref(intr->src[1]);
  struct glsl_cmat_description dst_desc =
      *glsl_get_cmat_description(dst_deref->type);
  struct glsl_cmat_description src_desc =
      *glsl_get_cmat_description(src_deref->type);
  nir_def *src = gnm_nir_load_cmat(b, params, intr->src[1].ssa);
  ...
  if (transpose) {
    /* NV_cmat2 only support acc -> b transpose, but we can handle any transpose
     * except acc -> acc. */
    ...
    dst_element_type = glsl_apply_signedness_to_base_type(
        dst_element_type, cmat_signed_mask & NIR_CMAT_RESULT_SIGNED);
    src_element_type = glsl_apply_signedness_to_base_type(
        src_element_type, cmat_signed_mask & NIR_CMAT_A_SIGNED);
  }

  unsigned dst_mul = gnm_nir_cmat_length_mul(dst_desc, params);
  unsigned src_mul = gnm_nir_cmat_length_mul(src_desc, params);
  ...
  if (gnm_nir_cmat_bits(src_desc) <= gnm_nir_cmat_bits(dst_desc))
    src = convert_use(b, src, src_use, dst_use, params);

  nir_def *ret =
      convert_base_type(b, src, src_element_type, dst_element_type, sat);
...
static bool lower_cmat_unary_op(nir_builder *b, nir_intrinsic_instr *intr,
                                const lower_cmat_params *params) {
  b->fp_math_ctrl = nir_intrinsic_fp_math_ctrl(intr);
  nir_def *src = gnm_nir_load_cmat(b, params, intr->src[1].ssa);
  nir_op op = nir_intrinsic_alu_op(intr);
  nir_def *ret = nir_build_alu1(b, op, src);
  nir_store_deref(b, nir_src_as_deref(intr->src[0]), ret,
...
static bool lower_cmat_scalar_op(nir_builder *b, nir_intrinsic_instr *intr,
                                 const lower_cmat_params *params) {
  b->fp_math_ctrl = nir_intrinsic_fp_math_ctrl(intr);
  nir_def *src1 = gnm_nir_load_cmat(b, params, intr->src[1].ssa);
  nir_op op = nir_intrinsic_alu_op(intr);
...
static bool lower_cmat_binary_op(nir_builder *b, nir_intrinsic_instr *intr,
                                 const lower_cmat_params *params) {
  b->fp_math_ctrl = nir_intrinsic_fp_math_ctrl(intr);
  nir_def *src1 = gnm_nir_load_cmat(b, params, intr->src[1].ssa);
  nir_def *src2 = gnm_nir_load_cmat(b, params, intr->src[2].ssa);
  nir_op op = nir_intrinsic_alu_op(intr);
...
static bool lower_cmat_bitcast(nir_builder *b, nir_intrinsic_instr *intr,
                               const lower_cmat_params *params) {
  nir_def *src1 = gnm_nir_load_cmat(b, params, intr->src[1].ssa);
  nir_store_deref(b, nir_src_as_deref(intr->src[0]), src1,
                  nir_component_mask(src1->num_components));
  nir_instr_remove(&intr->instr);
  return true;
}

static bool lower_cmat_copy(nir_builder *b, nir_intrinsic_instr *intr) {
  nir_build_copy_deref(b, intr->src[0].ssa, intr->src[1].ssa);
  nir_instr_remove(&intr->instr);
  return true;
}

static bool lower_cmat_reduce_finish_call(nir_builder *b,
  ...
  struct glsl_cmat_description src_desc =
      *glsl_get_cmat_description(src0_deref->type);

  nir_def *src0 = gnm_nir_load_cmat(b, params, call->params[1].ssa);
  nir_def *src1 = gnm_nir_load_cmat(b, params, call->params[2].ssa);

  unsigned length = gnm_nir_cmat_length(src_desc, params);
  unsigned mul = gnm_nir_cmat_length_mul(src_desc, params);
  ...
  if (mul > 1) {
    for (unsigned i = 0; i < length; ++i)
      if (i % mul != 0)
        vars[i] = nir_undef(b, 1, gnm_nir_cmat_bits(src_desc));
  }

  if (reduce & NIR_CMAT_REDUCE_COLUMN) {
    nir_variable *col_tmp = nir_local_variable_create(
        b->impl, glsl_get_bare_type(fnptr->params[0].type), "col_tmp");
    ...
    /* All of the columns contain the same data, but not the rows, so across all
     * components. */
    for (unsigned i = 0; i < length / mul; ++i) {
      nir_def *row0_accum = nir_channel(b, src0, i * mul);
      nir_def *row1_accum = nir_channel(b, src1, i * mul);

      nir_variable *row_tmp = nir_local_variable_create(
          b->impl, glsl_get_bare_type(fnptr->params[0].type), "row_tmp");
...
static bool lower_cmat_reduce_call(nir_builder *b, nir_cmat_call_instr *call,
                                   const lower_cmat_params *params) {
  nir_deref_instr *dst_deref = nir_src_as_deref(call->params[0]);
  nir_deref_instr *src_deref = nir_src_as_deref(call->params[1]);
  struct glsl_cmat_description src_desc =
      *glsl_get_cmat_description(src_deref->type);
  nir_def *src = gnm_nir_load_cmat(b, params, call->params[1].ssa);
  nir_function *fnptr = call->callee;
...
static bool lower_cmat_reduce_2x2_call(nir_builder *b,
                                       nir_cmat_call_instr *call,
                                       const lower_cmat_params *params) {
  nir_deref_instr *dst_deref = nir_src_as_deref(call->params[0]);
  nir_deref_instr *src_deref = nir_src_as_deref(call->params[1]);
  ...
  for (unsigned m = 0; m < 4; m++) {
    nir_def *src = gnm_nir_load_cmat(b, params, call->params[m + 1].ssa);
    for (unsigned i = 0; i < length / mul; i++) {
      src_components[m][i] = nir_channel(b, src, i * mul);
    }
  }

  nir_variable *qd_tmp = nir_local_variable_create(
      b->impl, glsl_get_bare_type(fnptr->params[0].type), "qd_tmp");
  ...
  for (unsigned m = 0; m < 4; m++) {
    for (unsigned i = 0; i < length / mul / 2; i++) {
      if (params->gfx_level >= GFX12) {
        ...
        nir_call(b, fnptr, &qd_tmp_deref->def, src_components[m][i * 2],
                 src_components[m][i * 2 + 1]);
        src_components[m][i] = nir_load_deref(b, qd_tmp_deref);
      } else {
        /* The neighboring row is in the upper 16 lanes */
        for (unsigned j = 0; j < 2; j++) {
          nir_def *other_row =
              shuffle_xor_imm(b, src_components[m][i * 2 + j], 16);
          nir_call(b, fnptr, &qd_tmp_deref->def, src_components[m][i * 2 + j],
                   other_row);
          src_components[m][i * 2 + j] = nir_load_deref(b, qd_tmp_deref);
        }

        /* Fill the duplicated space with the rows from the next VGPR */
        src_components[m][i] = nir_bcsel(b, low16, src_components[m][i * 2],
                                         src_components[m][i * 2 + 1]);
      }
  ...
  nir_def *even =
      nir_inverse_ballot_imm(b, 0x5555555555555555, params->wave_size);
  ...
  /* In each row, every second element needs to be moved to the end, and the
   * others to the front. This can be done with v_permlane16_b32.
   */
  ...
  if (params->gfx_level >= GFX12) {
    /* For GFX12, we still have to swap the row(s) in upper half coming from the
     * bottom two matrices with low row(s) in the from other two matrices.
     */
    nir_def *low = nir_inverse_ballot_imm(
        b, BITFIELD64_MASK(params->wave_size / 2), params->wave_size);
    for (unsigned i = 0; i < length / mul / 2; i++) {
  ...
  if (params->wave_size == 64) {
    ...
    nir_def *keep =
        nir_inverse_ballot_imm(b, 0xffff00000000ffff, params->wave_size);
    for (unsigned i = 0; i < length / mul; i++)
  ...
  for (int i = length / mul - 1; i >= 0; i--) {
    ...
    for (unsigned j = 1; j < mul; j++)
      vars[i * mul + j] = nir_undef(b, 1, gnm_nir_cmat_bits(src_desc));
  }
...
static bool lower_cmat_deref(nir_deref_instr *deref,
                             struct hash_table *type_map,
                             const lower_cmat_params *params) {
  const struct glsl_type *new_type =
      gnm_nir_translate_matrix_type(deref->type, type_map, params);
...
static bool lower_cmat_per_element_op(nir_builder *b, nir_cmat_call_instr *call,
                                      const lower_cmat_params *params) {
  nir_def *src = gnm_nir_load_cmat(b, params, call->params[3].ssa);
  nir_deref_instr *dst_deref = nir_src_as_deref(call->params[0]);
  nir_function *fnptr = call->callee;
  ...
  nir_variable *elem_tmp = nir_local_variable_create(
      b->impl, glsl_get_cmat_element(dst_deref->type), "elemtmp");
  ...
  for (unsigned i = 0; i < length / mul; i++) {
    ...
    for (unsigned p = 4; p < call->num_params; p++) {
      ...
      if (deref) {
        if (glsl_type_is_cmat(deref->type)) {
          def = nir_build_load_deref(b, gnm_nir_cmat_length(desc, params),
                                     gnm_nir_cmat_bits(desc), def);
    ...
    vars[i * mul] = nir_build_load_deref(b, 1, gnm_nir_cmat_bits(desc),
                                         &elem_deref->def, 0);
  }
...
bool gnm_nir_lower_cooperative_matrix(nir_shader *shader,
  ...
  nir_foreach_variable_with_modes(var, shader, nir_var_shader_temp) {
    const struct glsl_type *new_type =
        gnm_nir_translate_matrix_type(var->type, type_map, &params);
  ...
  nir_foreach_block_reverse(block, func->impl) {
    nir_foreach_instr_reverse_safe(instr, block) {
      ...
      case nir_instr_type_deref:
        progress |=
            lower_cmat_deref(nir_instr_as_deref(instr), type_map, &params);
        break;
  ...
  nir_foreach_function_impl(fnim, shader) nir_progress(progress, fnim, 0);

  return nir_progress(progress, func->impl, 0);
}

static bool apply_component_mods(nir_scalar *comp, unsigned num_comps,
                                 unsigned stride, nir_op alu_op) {
  for (unsigned i = 0; i < num_comps; i++) {
...
static bool opt_cmat_modifiers(nir_builder *b, nir_intrinsic_instr *intrin,
                               enum amd_gfx_level gfx_level, unsigned src_idx) {
  unsigned length_mul =
      src_idx == 2 && intrin->src[2].ssa->bit_size == 16 && gfx_level < GFX12
          ? 2
          : 1;
...
static bool opt_cmat(nir_builder *b, nir_intrinsic_instr *intrin, void *data) {
...
bool gnm_nir_opt_cooperative_matrix(nir_shader *shader,
                                    enum amd_gfx_level gfx_level) {
  return nir_shader_intrinsics_pass(shader, opt_cmat, nir_metadata_control_flow,
                                    &gfx_level);
}
